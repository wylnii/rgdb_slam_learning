...
int main( int argc, char** argv )
{
    ...
    *******************************/
    ...
    {
        cout<<RESET"Reading files : "<< currIndex << " (" << rgbList[currIndex] << " " << depthList[currIndex] << ")" << endl;
...
CHECK_RESULT checkKeyframes( FRAME& f1, FRAME& f2, g2o::SparseOptimizer& opti, bool is_loops)
{
    ...
    if ( is_loops == false )
    {
        cout <<RESET "goodMatchs: " << result.goodMatchs << " inliers: " << result.inliers << " "<<flush;
        if(show_matches)
            cv::waitKey();
    }
