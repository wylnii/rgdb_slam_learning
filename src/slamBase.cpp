...
{
    if(detector == "ORB") //use ORB_SLAM2::ORBextractor
    {
        ORB_SLAM2::ORBextractor extractor(800,1.2f,8,20,7);
        extractor(frame.rgb, cv::Mat(), frame.kp, frame.desp);
        return;
    }
...
RESULT_OF_PNP estimateMotion( FRAME& frame1, FRAME& frame2, CAMERA_INTRINSIC_PARAMETERS& camera )
{
    ...
    if (goodMatches.size() <= atoi( pd.getData( "min_good_match" ).c_str() ))
    {
        result.inliers = -1;
        return result;
    }
    ...
    // 求解pnp
    cv::solvePnPRansac( pts_obj, pts_img, cameraMatrix, cv::Mat(), rvec, tvec, false, 100, 1.0, 100, inliers, cv::EPNP);
...
FRAME readFrame(int index, vector<string> &rgbList, vector<string> &depthList, ParameterReader& pd)
{
    ...
    int pos = rgbList[index].rfind('.');
    if(pos >= 0)
        f.timestamp = rgbList[index].substr(0, pos); //添加时间戳(TUM)
    return f;
}
